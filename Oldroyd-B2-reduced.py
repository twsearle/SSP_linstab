...
#   (Reduced profiles)
#   Last modified: Thu 18 Apr 2013 16:58:57 BST
#----------------------------------------------------------------------------#
""" Perform Linear stability analysis to find eigenvalues for the stability 
of the streaky flow. Set all components of the profile to zero except first
5 Fourier Modes."""
...
if Nselect == 0:
    ...
    W[(N+Nselect+1)*M:]                   = 0 
    conxx[:(N-Nselect)*M]                 = 0
    conxx[(N-Nselect+1)*M:(N+Nselect)*M]  = 0  
    conxx[(N+Nselect+1)*M:]               = 0  
    conyy[:(N-Nselect)*M]                 = 0
    conyy[(N-Nselect+1)*M:(N+Nselect)*M]  = 0  
    conyy[(N+Nselect+1)*M:]               = 0  
    conzz[:(N-Nselect)*M]                 = 0
    conzz[(N-Nselect+1)*M:(N+Nselect)*M]  = 0
    conzz[(N+Nselect+1)*M:]               = 0  
    conxy[:(N-Nselect)*M]                 = 0
    conxy[(N-Nselect+1)*M:(N+Nselect)*M]  = 0
    conxy[(N+Nselect+1)*M:]               = 0  
    conxz[:(N-Nselect)*M]                 = 0
    conxz[(N-Nselect+1)*M:(N+Nselect)*M]  = 0
    conxz[(N+Nselect+1)*M:]               = 0  
    conyz[:(N-Nselect)*M]                 = 0
    conyz[(N-Nselect+1)*M:(N+Nselect)*M]  = 0
    conyz[(N+Nselect+1)*M:]               = 0  
...
savetxt('ev-k{k}{file}-redn{Nselect}.dat'.format(k=k, file=filename[:-7],
