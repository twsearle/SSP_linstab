...
#   Last modified: Mon 11 Mar 2013 16:33:51 GMT
#----------------------------------------------------------------------------#
...
EGreal = config.getfloat('settings', 'Eig Guess Real')
EGimag = config.getfloat('settings', 'Eig Guess Imag')

fp.close()

eigGuess = EGreal + EGimag*1.j
print 'The guess for the eigenvalue is: {0}, at k = {1}'.format(
                                    eigGuess, k)
...
# Use library function to solve for first few eigenvalues
print 'in sparse.linalg.eigs', (time.time()-startTime)
eigenvals = sparselinalg.eigs(equations_matrix, k=4, M=RHS, sigma = eigGuess\
