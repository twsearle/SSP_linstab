...
parser.add_argument('-comp', '--comp', default='dcxx', 
                    help='change to alter the component plotted.')
...
options1 = {'dcxx': dcxx, 'dcyy': dcyy, 'dczz': dczz, 'dcxy': dcxy, 'dcxz':
            dcxz, 'dcyz': dcyz}
options2 = {'dcxx': dcxx2, 'dcyy': dcyy2, 'dczz': dczz2, 'dcxy': dcxy2, 'dcxz':
            dcxz2, 'dcyz': dcyz2}
...
for n in range(N,2*N+1):
    ...
    var = options1[args.comp]
    var2 = options2[args.comp]
...
plt.savefig('pertb-{comp}-{pf}.pdf'.format(comp=args.comp, pf='comparsion'))
plt.show()
