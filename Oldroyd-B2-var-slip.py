...
#   Last modified: Thu 06 Mar 2014 22:32:39 GMT
#----------------------------------------------------------------------------#
...
cfgalpha = config.getfloat('settings', 'alpha')
...
argparser.add_argument("-alpha", type=float, default=cfgalpha,
                help='Override alpha from the config file')
...
alpha = {7}
----------------------------------------
""". format(N, M, Re, beta, Weiss, Amp, k, alpha)
...
def mk_bigM():
    ...
    for i in range((2*N+1)):
        bigM[M*(i+1)-2,:] = hstack((zeros(M*i), VARTOP, zeros(10*vLen-M*(i+1))))
        bigM[M*(i+1)-1,:] = hstack((zeros(M*i), VARBOT, zeros(10*vLen-M*(i+1))))
    del i
    #v
...
VARTOP = alpha*DERIVTOP + (1.-alpha)*BTOP
VARBOT = alpha*DERIVBOT + (1.-alpha)*BBOT
...
if args.evecs:
    ...
    savetxt('ev-k{k}{fn}-a{a}.dat'.format(k=k, fn=filename[:-7], a=alpha
                                                     ), eigarray)

    pickle.dump((du,dv,dw,dp,dcxx,dcyy,dczz,dcxy,dcxz,dcyz),
                open('full-evecs-k{k}{fn}-a{alpha}.pickle'.format(k=k,fn=filename[:-7], a=alpha), 'w'))
    ...
    savetxt('ev-k'+str(k)+filename[:-7]+'-a'+str(alpha)+'.dat', eigarray)
